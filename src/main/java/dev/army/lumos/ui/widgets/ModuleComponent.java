...
public class ModuleComponent extends ContainerComponent {
    ...
    public void render(LumosDrawContext ctx) {
        ctx.fill(x, y, x + width, y + height, (module.getMetadata().hide_enabled() || !module.isEnabled()) ? 0xAA333333 : 0x33FF33FF);
        TextRenderer tr = MinecraftClient.getInstance().textRenderer;
        ctx.drawText(tr, name, x + padding, y + padding, -1, false);
        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, Identifier.of(LumosClient.getModId(), "textures/ui/dropdown" + ((this.expanded) ? "_rotated":"") + ".png"), x + width - height / 2 - 5, y + height / 2 - 5, 0, 0, 10, 10, 10, 10, 0xFFFFFFFF);
    }

    @Override
    public void mouseClicked(Click click) {
        if (!LumosMath.inBounds((int) click.x(), (int) click.y(), x, y, x + width, y + base_height)) return;
